...
//->左右切换
document.addEventListener("touchmove", function (e) {//要给页面中的某一个元素绑定touchmove行为，需要提前把浏览器touchmove默认的行为给注释掉
    e.preventDefault();
}, false);
...
function start(e) {
    ...
    //->记录当前元素的开始的坐标和left值
    var touchPoint = e.touches[0];//建议都用touches
...
function isSwipe(strX, endX, strY, endY) {
    return Math.abs(endX - strX) > 30 || Math.abs(endY - strY) > 30;//30px为滑动偏差
}
/**
 * 检测滑动方向
 * @param strX  x起始位置
